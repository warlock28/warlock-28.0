...
import { ExternalLink, Award, ChevronLeft, ChevronRight, Loader2, AlertCircle } from 'lucide-react';
    ...
    <section id={sectionId} className="py-12 sm:py-16 lg:py-20 relative overflow-hidden">
      ...
      {!loading && error && (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground text-sm">Failed to load certifications. Please try again later.</p>
        </div>
      )}

      {/* Mobile: Horizontal Scroll with Navigation */}
      {!loading && !error && displayCertifications.length > 0 && (
        <motion.div
      ...
      {!loading && !error && displayCertifications.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">{isFeaturedVariant ? 'No featured certifications added yet.' : 'No certifications found.'}</p>
        </div>
      )}
